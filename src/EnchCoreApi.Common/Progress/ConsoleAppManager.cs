...
public class ConsoleAppManager : IDisposable {
    ...
    public ConsoleAppManager(string appName, bool waitforExit = false) {
        ...
        //this.process.StartInfo.StandardInputEncoding = Console.InputEncoding = Encoding.UTF8;
        this.process.StartInfo.StandardOutputEncoding = Console.OutputEncoding = Encoding.UTF8;
    ...
    public void Dispose() {
        this.ErrorTextReceived = null;
        this.ProcessExited = null;
        this.StandartTextReceived = null;
    }
    ...
    private void ReadOutputAsync() {
        while (this.process.HasExited == false) {
            this.OnStandartTextReceived(process.StandardOutput.ReadLine() ?? "");
            Thread.Sleep(1);
        }
        this.Running = false;
    }

    private void ReadOutputErrorAsync() {
        do {
            this.OnErrorTextReceived(process.StandardError.ReadLine() ?? "");
            Thread.Sleep(1);
        }
        while (this.process.HasExited == false);
    }
